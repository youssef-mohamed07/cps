...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> { const { locale, serviceSlug } = await params; if (!isLocale(locale)) return {}; const service = getServiceArchitecture(serviceSlug); if (!service) return {}; await ensureSiteConfig(); return buildPageMetadata({ path: `/services/${serviceSlug}/catalogue`, locale, fallbackTitle: `CPS — ${localizeText(service.catalogue.title, locale)}`, fallbackDescription: localizeText(service.catalogue.support, locale), fallbackOgImage: service.image }); }
